...
const processTurnsSequentially = async () => {
  ...
  <div className={`min-h-screen flex flex-col ${theme.bg}`}>
    ...
    {/* Main Content */}
    <div className={`flex flex-row w-full ${theme.bg}`}>
      ...
      {/* Center Panel - Loading / No Wallet / Battle Scene */}
      <div className="flex-none w-[70%] flex items-center justify-center relative">
        {!wallet?.address ? (
          ...
          <>
            ...
            <div className="relative w-full max-w-[100vw]" style={{ aspectRatio: "16 / 9", maxHeight: "60vh" }}>
              <BattleScene
                challenger={activeBattle.challenger}
                accepter={activeBattle.accepter}
      ...
      <div className="flex-none w-[20%] flex flex-col items-center justify-start">
    ...
    {/* Bottom Panel - Moves */}
    {activeBattle && (
<div className={`flex flex-row w-full px-6 gap-4 mt-4 ${theme.bg}`}>
  {/* Player Moves */}
  <div className="flex-1">
    <div className={`p-4 rounded-lg ${theme.container} ${theme.border} backdrop-blur-md`}>
        ...
        <div className="grid grid-cols-2 gap-2 relative">
          {Object.entries(activeBattle.challenger.moves).map(([moveName, move]) => (
            <button
              ...
              disabled={
                isUpdating || movesDisabled || activeBattle.status === "ended" || move.count === 0
              }
              className={`w-full p-2 rounded-lg font-medium text-left transition-all duration-300 min-h-[80px]
                ...
                ${
                  activeBattle.status === "ended" || movesDisabled || move.count === 0
            ...
            >
              ...
              <div className="text-sm mt-1">
                <div className="grid grid-cols-3 grid-rows-2 gap-1 max-w-[200px] min-h-[32px]">
                  {move.damage !== 0 && <span>⚔️ {move.damage > 0 ? "+" : ""}{move.damage}</span>}
                  {move.attack !== 0 && <span>💪 {move.attack > 0 ? "+" : ""}{move.attack}</span>}
                  {move.defense !== 0 && <span>🛡️ {move.defense > 0 ? "+" : ""}{move.defense}</span>}
                  {move.speed !== 0 && <span>⚡ {move.speed > 0 ? "+" : ""}{move.speed}</span>}
                  {move.health !== 0 && <span>❤️ {move.health > 0 ? "+" : ""}{move.health}</span>}
                  {[...Array(6 - [move.damage, move.attack, move.defense, move.speed, move.health].filter((v) => v !== 0).length)]
                    .map((_, i) => <span key={i} className="invisible">placeholder</span>)}
                </div>
              </div>
          ...
          {Object.values(activeBattle.challenger.moves).every((move) => move.count === 0) && (
            <button
              ...
              className={`absolute inset-0 m-auto w-32 h-32 p-2 rounded-lg font-medium transition-all duration-300 
                ...
                ${
                  activeBattle.status === "ended" || movesDisabled
                    ? "opacity-50 cursor-not-allowed"
                    : ""
                }
