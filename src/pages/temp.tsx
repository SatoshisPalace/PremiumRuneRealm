                      ...
                      <div className="relative">
                        ...
                        <div className="grid grid-cols-2 gap-2 relative">
                          {Object.entries(activeBattle.challenger.moves).map(([moveName, move]) => {
                            const isPlayerChallenger = wallet?.address === activeBattle.challenger.address;
                            return (
                              <button
                                key={moveName}
                                onClick={() => handleAttack(moveName)}
                                disabled={!isPlayerChallenger || isUpdating || movesDisabled || activeBattle.status === 'ended' || move.count === 0}
                                className={`w-full p-2 rounded-lg font-medium text-left transition-all duration-300 min-h-[80px]
                                  ${getMoveColor(moveName, move)} ${!isPlayerChallenger ? 'opacity-75' : 'hover:brightness-110'}
                          ...
                          {Object.values(activeBattle.challenger.moves).every(move => move.count === 0) && (
                            <button
                              ...
                              disabled={!isPlayerChallenger || isUpdating || movesDisabled || activeBattle.status === 'ended'}
                      ...
                      <div className="relative">
                        <div className="grid grid-cols-2 gap-2 relative">
                          {Object.entries(activeBattle.accepter.moves).map(([moveName, move]) => {
                            const isPlayerAccepter = wallet?.address === activeBattle.accepter.address;
                            return (
                              <button
                                key={moveName}
                                onClick={() => handleAttack(moveName)}
                                disabled={!isPlayerAccepter || isUpdating || movesDisabled || activeBattle.status === 'ended' || move.count === 0}
                                className={`w-full p-2 rounded-lg font-medium text-left transition-all duration-300 min-h-[80px]
                                  ${getMoveColor(moveName, move)} ${!isPlayerAccepter ? 'opacity-75' : 'hover:brightness-110'}
                                  text-white relative overflow-hidden group flex flex-col justify-between`}
                              >
                                <div className="flex justify-between items-center relative">
                                  <span className="capitalize">{moveName}</span>
                                ...
                                <div className="text-sm mt-1">
                                  <div className="grid grid-cols-3 grid-rows-2 gap-1 max-w-[200px] min-h-[32px]">
                                    ...
                                    {/* Add empty spans to maintain 3x2 grid layout */}
                                    {[...Array(6 - [move.damage, move.attack, move.defense, move.speed, move.health].filter(v => v !== 0).length)].map((_,
