                        ...
                        <div className="grid grid-cols-2 gap-2 relative">
                          {Object.entries(activeBattle.challenger.moves).map(([moveName, move]) => (
                            <button
                              ...
                              disabled={wallet?.address !== activeBattle.challenger.address || isUpdating || movesDisabled || activeBattle.status === 'ended' || move.count === 0}
                              className={`w-full p-2 rounded-lg font-medium text-left transition-all duration-300 min-h-[80px]
                                ${getMoveColor(moveName, move)} ${wallet?.address !== activeBattle.challenger.address ? 'opacity-75' : 'hover:brightness-110'}
                          ...
                          {Object.values(activeBattle.challenger.moves).every(move => move.count === 0) && (
                            <button
                              ...
                              disabled={wallet?.address !== activeBattle.challenger.address || isUpdating || movesDisabled || activeBattle.status === 'ended'}
                        ...
                        <div className="grid grid-cols-2 gap-2 relative">
                          {Object.entries(activeBattle.accepter.moves).map(([moveName, move]) => (
                            <button
                              key={moveName}
                              onClick={() => handleAttack(moveName)}
                              disabled={wallet?.address !== activeBattle.accepter.address || isUpdating || movesDisabled || activeBattle.status === 'ended' || move.count === 0}
                              className={`w-full p-2 rounded-lg font-medium text-left transition-all duration-300 min-h-[80px]
                                ${getMoveColor(moveName, move)} ${wallet?.address !== activeBattle.accepter.address ? 'opacity-75' : 'hover:brightness-110'}
                                text-white relative overflow-hidden group flex flex-col justify-between`}
                            >
                              <div className="flex justify-between items-center relative">
                                <span className="capitalize">{moveName}</span>
