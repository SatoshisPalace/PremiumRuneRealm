...
Handlers.add("ReturnFrom-MISSION", Handlers.utils.hasMatchingTag("Action", "ReturnFrom-MISSION"), function(msg)
...
Handlers.add("ReturnFrom-PLAY", Handlers.utils.hasMatchingTag("Action", "ReturnFrom-PLAY"), function(msg)
...
Handlers.add("GetPurchaseOptions", Handlers.utils.hasMatchingTag("Action", "GetPurchaseOptions"), function(msg)
    ao.send({
        Target = msg.From,
        Data = json.encode({
...
Handlers.add("GetFactions", Handlers.utils.hasMatchingTag("Action", "GetFactions"), function(msg)
    local factionsWithStats = {}

    for _, faction in ipairs(AvailableFactions) do
        local memberCount, monsterCount, members, avgLevel, totalFed, totalPlay, totalMission = GetFactionStats(
            faction.name)
...
Handlers.add('GetUserSkin', Handlers.utils.hasMatchingTag('Action', 'GetUserSkin'), function(msg)
...
Handlers.add('GetAuthorizedUsers', Handlers.utils.hasMatchingTag('Action', 'GetAuthorizedUsers'), function(msg)
    ao.send({
        Target = msg.From,
...
Handlers.add('UpdateSprite', Handlers.utils.hasMatchingTag('Action', 'UpdateSprite'), function(msg)
...
Handlers.add('SetFaction', Handlers.utils.hasMatchingTag('Action', 'SetFaction'), function(msg)
...
Handlers.add("CreditNoticeHandler", Handlers.utils.hasMatchingTag("Action", "Credit-Notice"), function(msg)
    print("credit-notice")
    ...
    if msg.Tags["X-Action"] == "BATTLE" then
        ...
        if monster.energy < monster.activities.battle.energyCost or monster.happiness <
            monster.activities.battle.happinessCost then
    ...
    if msg.Tags["X-Action"] == "MISSION" then
        ...
        if monster.energy < monster.activities.mission.energyCost or monster.happiness <
            monster.activities.mission.happinessCost then
            print("Monster doesn't have enough energy or happiness")
            return
        end
...
Handlers.add("CheckUnlocked", Handlers.utils.hasMatchingTag("Action", "CheckUnlocked"), function(msg)
    local address = ao.id
...
-- Handle CheckUnlocked action
Handlers.add("CheckSkin", Handlers.utils.hasMatchingTag("Action", "CheckSkin"), function(msg)
    local address = ao.id
    if msg.Tags.Address then
        address = msg.Tags.Address
    end
...
Handlers.add("CheckFaction", Handlers.utils.hasMatchingTag("Action", "CheckFaction"), function(msg)
    local address = ao.id
    if msg.Tags.Address then
        address = msg.Tags.Address
    end
...
Handlers.add('BulkImportAddresses', Handlers.utils.hasMatchingTag('Action', 'BulkImportAddresses'), function(msg)
    if not ensureAdmin(msg) then
        return
    end
...
Handlers.add('RemoveUser', Handlers.utils.hasMatchingTag('Action', 'RemoveUser'), function(msg)
    if not ensureAdmin(msg) then
        return
    end
...
Handlers.add('AdoptMonster', Handlers.utils.hasMatchingTag('Action', 'AdoptMonster'), function(msg)
...
Handlers.add('GetUserMonster', Handlers.utils.hasMatchingTag('Action', 'GetUserMonster'), function(msg)
...
Handlers.add("LevelUpMonster", Handlers.utils.hasMatchingTag("Action", "LevelUp"), function(msg)
    local userId = msg.From
    ...
    if totalPoints ~= 10 or attackPoints > 5 or defensePoints > 5 or speedPoints > 5 or healthPoints > 5 then
        ao.send({
            Target = msg.From,
            Data = json.encode({
                status = "error",
...
Handlers.add('GetUserInfo', Handlers.utils.hasMatchingTag('Action', 'GetUserInfo'), function(msg)
    local targetWallet = msg.Tags.Wallet or msg.From
...
Handlers.add('SetUserStats', Handlers.utils.hasMatchingTag('Action', 'SetUserStats'), function(msg)
    if not ensureAdmin(msg) then
        return
    end
    ...
    if newStats.level ~= nil then
        monster.level = newStats.level
    end
    if newStats.exp ~= nil then
        monster.exp = newStats.exp
    end
    if newStats.attack ~= nil then
        monster.attack = newStats.attack
    end
    if newStats.defense ~= nil then
        monster.defense = newStats.defense
    end
    if newStats.speed ~= nil then
        monster.speed = newStats.speed
    end
    if newStats.health ~= nil then
        monster.health = newStats.health
    end
    if newStats.energy ~= nil then
        monster.energy = math.min(100, newStats.energy)
    end
    if newStats.happiness ~= nil then
        monster.happiness = math.min(100, newStats.happiness)
    end
    ...
    if newStats.faction ~= nil then
        UserFactions[targetWallet] = {
            faction = newStats.faction
        }
    end
    if newStats.image ~= nil then
        monster.image = newStats.image
    end
    if newStats.name ~= nil then
        monster.name = newStats.name
    end
...
Handlers.add("AdjustAllMonsters", Handlers.utils.hasMatchingTag("Action", "AdjustAllMonsters"), function(msg)
    if not ensureAdmin(msg) then
        return
    end
...
Handlers.add("ReturnFrom-BATTLE", Handlers.utils.hasMatchingTag("Action", "ReturnFrom-BATTLE"), function(msg)
    if not ensureAdmin(msg) then
        return
    end
...
Handlers.add("OpenLootBox", Handlers.utils.hasMatchingTag("Action", "OpenLootBox"), function(msg)
...
-- Handler for returning from battle
Handlers.add("GetLootBox", Handlers.utils.hasMatchingTag("Action", "GetLootBox"), function(msg)

    print("Getting loot box")
    local userId = msg.Tags.UserId
    ao.send({
        Target = userId,
        Data = json.encode({UserLootBoxes[userId]})
    })
end)
